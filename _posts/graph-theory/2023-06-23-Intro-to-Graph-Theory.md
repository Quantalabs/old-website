...
<div style="max-width: 100%;">
  <iframe src="/assets/interactives/graph-theory/bfsinteractive.html" style="border: none; width: 100%; height: 500px;"></iframe>
</div>
...
Okay, finally the part you have been waiting for- though it might be a bit dissapointing. First lets create a graph from the seven bridges, like below, where each mass of land is a node, and each edge is a bridge.

For a eulerian path to exist, a path that uses each edge only once, is possible only if there are 2 or 0 edges of odd degree. If there are 2, they must be the start and end nodes, as when entering and exiting a node, you must use 2 different edges. For our graph, we have for nodes, which means we need all even degrees or 2 with an odd degree. But instead, we have all odd degrees. This means that we can't actually devise a path that uses each bridge only once- there is no solution to the problem.
